...
class bnsparams:
    def __init__(self,mc,M,eta,m1,m2,ra,dec,iota,phi,psi,idx,fmin,snr,SNR,lambda1,lambda2,sz1,sz2):
        self.mc = mc
        self.M = M
        self.eta = eta
        self.m1 = m1
...
def tukey(M,alpha=0.5): #cosine window
    """
    Tukey window code copied from scipy
    """
    n = np.arange(0, M)
    width = int(np.floor(alpha*(M-1)/2.0))#向下取整
    n1 = n[0:width+1]
    n2 = n[width+1:M-width-1]
    n3 = n[M-width-1:]
    
    # 前半部分的窗函数值
    w1 = 0.5 * (1 + np.cos(np.pi * (-1 + 2.0*n1/alpha/(M-1)))) #汉宁窗
    w2 = np.ones(n2.shape)
    # 后半部分的窗函数值
    w3 = 0.5 * (1 + np.cos(np.pi * (-2.0/alpha + 1 + 2.0*n3/alpha/(M-1))))
    # alpha为相对宽度
    w = np.concatenate((w1, w2, w3))
    # 矩形窗 适用于不需要担心频谱泄露的应用，但它会引入较多的高频噪声。
    # 汉宁窗 通过对信号两端的加权来减少频谱泄露，因此常用于平滑信号的傅里叶变换分析。
    return np.array(w[:M])

def parser(): #解析命令行参数，读取数据
    ...
    # arguments for reading in a data file 参数读取
    parser.add_argument('-N', '--Nsamp', type=int, default=7000, help='the number of samples')
    #parser.add_argument('-Nv', '--Nvalid', type=int, default=1500, help='the number of validation samples') #验证集
    #parser.add_argument('-Nt', '--Ntest', type=int, default=1500, help='the number of testing samples') # 测试集
    parser.add_argument('-Nn', '--Nnoise', type=int, default=25, help='the number of noise realisations per signal') #噪声样本
    ...
    parser.add_argument('-m', '--mdist', type=str, default='astro', help='mass distribution for training (astro,gh,metric)') #质量分布类型
    parser.add_argument('-z', '--seed', type=int, default=1, help='the random seed') #随机种子?

    return parser.parse_args()

def convert_beta(beta,fs,T_obs):# beta表示时间段在整个信号窗口中的占比
    """
    ...
    return low_idx,high_idx #起始和结束的采样点索引

def gen_noise(fs,T_obs,psd):
    """
    Generates noise from a psd 生成噪声信号
    """
...
def get_snr(data,T_obs,fs,psd,fmin):
    ...
    df = 1.0/T_obs
    dt = 1.0/fs
    fidx = int(fmin/df) #起始频率

    win = tukey(N,alpha=1.0/8.0)
    idx = np.argwhere(psd>0.0)
    invpsd = np.zeros(psd.size)
    invpsd[idx] = 1.0/psd[idx] #倒数

    xf = np.fft.rfft(data*win)*dt #快速fft 返回频域信号
    SNRsq = 4.0*np.sum((np.abs(xf[fidx:])**2)*invpsd[fidx:])*df #计算SNR平方，信号的频域功率与 PSD 的倒数相乘
    return np.sqrt(SNRsq)

def whiten_data(data,duration,sample_rate,psd,flag='td'):
    """
    Takes an input timeseries and whitens it according to a psd 白化处理
    """
...
def gen_masses(m_min=1.0,M_max=3.0,mdist='astro',verbose=True):
    """
    function returns a pair of masses drawn from the appropriate distribution
    mdist=astro/gh/metric
    """
    flag = False
    if mdist=='astro':
        if verbose:
        ...
        while not flag:
            ...
            m12[1] = np.random.uniform(1.0,2.6,1)
            m12[0] = m12[1]*q #随机生成
            flag = True if (np.all(m12<3.0)) and (np.all(m12>1.0)) and (m12[0]>=m12[1]) else False
        eta = m12[0]*m12[1]/(m12[0]+m12[1])**2
        mc = np.sum(m12)*eta**(3.0/5.0)
        return m12, mc, eta
...
def get_fmin(M,eta,dt,verbose):
    """
    Compute the instantaneous frequency given a time till merger 合并时的频率
    """
    M_SI = M*MSUN_SI
...
def gen_par(fs,T_obs,mdist='astro',beta=[0.75,0.95],verbose=True):
    ...
    m_min = 1.00         # rest frame component masses
    M_max = 2.60       # rest frame total mass
    log_m_max = np.log(M_max - m_min)

    m12, mc, eta = gen_masses(m_min,M_max,mdist=mdist,verbose=verbose)
    M = np.sum(m12)
    if verbose:
        print('{}: selected bns masses = {},{} (chirp mass = {})'.format(time.asctime(),m12[0],m12[1],mc))

    # generate iota 倾斜角
    iota = np.arccos(-1.0 + 2.0*np.random.rand())
    if verbose:
        print('{}: selected bns cos(inclination) = {}'.format(time.asctime(),np.cos(iota)))

    # generate polarisation angle 极化角
    psi = 2.0*np.pi*np.random.rand()
    if verbose:
        print('{}: selected bns polarisation = {}'.format(time.asctime(),psi))

    # generate reference phase 相位
    phi = 2.0*np.pi*np.random.rand()
    if verbose:
        print('{}: selected bns reference phase = {}'.format(time.asctime(),phi))

    # pick sky position - uniform on the 2-sphere 天球上的位置
    ra = 2.0*np.pi*np.random.rand()
    dec = np.arcsin(-1.0 + 2.0*np.random.rand())
    if verbose:
        print('{}: selected bns sky position = {},{}'.format(time.asctime(),ra,dec))
    ...
    if verbose:
        print('{}: selected bns peak amplitude time = {}'.format(time.asctime(),idx/fs))
    ...
    #generatel tidal parameters 
    lambda1 = float(np.random.uniform(((2.45e+05) * np.exp(-6.16 * m12[0])), ((6.45e+05) * np.exp(-4.386 * m12[0])), 1))
    lambda2 = float(np.random.uniform(lambda1, ((6.45e+05) * np.exp(-4.386 * m12[1])), 1))
    if verbose:
        print('{}: selected bns tidal deformability= {},{}'.format(time.asctime(), lambda1, lambda2))
    
    #generate spin parameters
    sz1 = float(np.random.uniform(-0.05,0.05,1))
    sz2= float(np.random.uniform(-0.05,0.05,1))
    if verbose:
        print('{}: selected bns spin= {},{}'.format(time.asctime(),sz1,sz2))

    # store params 
    par=bnsparams(mc, M, eta, m12[0], m12[1], ra, dec, np.cos(iota), phi, psi, idx, fmin, None, None, lambda1, lambda2, sz1, sz2)
...
def gen_bns(fs,T_obs,psds,snr=1.0,dets=['H1'],beta=[0.75,0.95],par=None,verbose=True):
    """
    generates a BBH timedomain signal 时域信号
    """
    N = T_obs * fs      # the total number of time samples
    dt = 1 / fs             # the sampling time (sec)
    f_low = 30.0            # lowest frequency of waveform (Hz)  修改
    amplitude_order = 0
    phase_order = 7
    approximant = lalsimulation.IMRPhenomPv2_NRTidalv2 #https://arxiv.org/abs/1905.06011   
    dist = 1e6*lal.PC_SI  # put it as 1 MPc

     # tidal deformability
    params = lal.CreateDict()
    lalsimulation.SimInspiralWaveformParamsInsertTidalLambda1(params, par.lambda1)
    lalsimulation.SimInspiralWaveformParamsInsertTidalLambda2(params, par.lambda2)
...
def sim_data(fs,T_obs,snr=1.0,dets=['H1'],Nnoise=25,size=1000,mdist='astro',beta=[0.75,0.95], verbose=True):
    ...
    """
    masses = []   #store mass distribution
    ...
    return [ts[idx], yval[idx]], temp,masses
