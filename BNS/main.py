...
class DatasetGenerator(Dataset):
    def __init__(self, fs=8192, T=1, snr=20,
                 ...
                 Nnoise=25, mdist='gh',beta=[0.75,0.95],
                 verbose=True):
        # 初始化函数，设置各种参数
        if verbose:
    ...
    def generate(self, Nblock, Nnoise=25, mdist='gh',beta=[0.75,0.95]):
        # 生成数据的函数
        ...
        self.strains = np.expand_dims(ts[0], 1)   # (nsample, 1, len(det), fs*T) #数据
        self.labels = ts[1] #标记
        self.masses = masses 
...
#
##############################################################################################################
def Conv1(in_planes, places, stride=4):
...
class MyNet(nn.Module):
    def __init__(self,blocks, num_classes=2, expansion = 4):
        ...
        self.maxpool=  nn.MaxPool2d(kernel_size=(1,1), stride=(1,4))
...
class CNN_l(nn.Module):
    def __init__(self):
        ...
        # 添加全连接层，输入维度为107776，输出维度为64
        self.layers.append(nn.Linear(107776, 2048))
        # 添加ELU激活函数，alpha参数为0.01
        self.layers.append(nn.ELU(0.01))
        # 添加Dropout层，丢弃率为0.5
        self.layers.append(nn.Dropout(0.5))
        self.layers.append(nn.Linear(2048, 64))
        self.layers.append(nn.ELU(0.01))
