...
class DatasetGenerator(Dataset):
    def __init__(self, fs=8192, T=1, snr=20,
        ...
        # safe = 1       # define the safe multiplication scale for the desired time length
        self.T *= safe
    ...
    def generate(self, Nblock, Nnoise=25, mdist='metric',beta=[0.75,0.95]):
        ...
        ts, par,masses= sim_data(self.fs, self.T, self.snr, self.detectors, Nnoise, size=Nblock, mdist=mdist,beta=beta, verbose=False)
        self.strains = np.expand_dims(ts[0], 1)   # (nsample, 1, len(det), fs*T) 
        self.labels = ts[1] #标记
        self.masses = masses 
        # self.strains = np.transpose(ts[0], (0, 2, 1))  # (nsample, len(det), 1, fs*T) 
...
#
##############################################################################################################
def Conv1(in_planes, places, stride=8):
    return nn.Sequential(
        nn.Conv2d(in_channels=in_planes,out_channels=places,kernel_size=(1,32),stride=(1,stride),padding=(0,3),bias=False),
...
class Bottleneck(nn.Module):
    def __init__(self,in_places,places, stride=1,downsampling=False, expansion = 4):
        ...
        self.bottleneck = nn.Sequential(
            ...
            nn.BatchNorm2d(places),
            nn.ELU(0.01),
            nn.Conv2d(in_channels=places, out_channels=places, kernel_size=(1,3), stride=(1,stride), padding=(0,1), bias=False),
            nn.BatchNorm2d(places),
            nn.ELU(0.01),
        ...
        if self.downsampling:
            self.downsample = nn.Sequential(
                nn.Conv2d(in_channels=in_places, out_channels=places*self.expansion, kernel_size=1, stride=(1,stride), bias=False),
                nn.BatchNorm2d(places*self.expansion)
            )
...
class MyNet(nn.Module):
    def __init__(self,blocks, num_classes=2, expansion = 4):
        ...
        self.layer2 = self.make_layer(in_places = 256,places=128, block=blocks[1], stride=4)
        self.layer3 = self.make_layer(in_places=512,places=256, block=blocks[2], stride=4)
        self.layer4 = self.make_layer(in_places=1024,places=512, block=blocks[3], stride=4)
 
        self.avgpool = nn.AvgPool2d((1,1), stride=(1,2))
...
class CNN_l(nn.Module):
    def __init__(self):
        # 初始化函数，设置网络的各种参数
        super(CNN_l, self).__init__() #调用父类的_init_函数
...
class LSTM_l(nn.Module):
    def __init__(self, input_size=2, hidden_size=10, num_layers=3, num_classes=2):
    ...
    def forward(self, x):
        ...
        rnn_output, (h_n, c_n) = self.rnn_layer(x, None) 
        output = self.linear_layer(rnn_output[:, -1, :])
        return output

def ResNet50():
    return MyNet([3, 4, 6, 3])
 
def ResNet101():
    return MyNet([3, 4, 23, 3])

def ResNet152():
    return MyNet([3, 8, 36, 3])
...
def load_model(checkpoint_dir=None,model_type=None):
    # 加载模型的函数，判断是否加载过
    ...
    if (checkpoint_dir is not None) and (Path(checkpoint_dir).is_dir()):
        ...
        train_loss_history = np.load(p / 'train_loss_history.npy').tolist()
...
def train(net, lr, nsample_perepoch, epoch, total_epochs,
    ...
    for epoch in range(epoch, epoch + total_epochs):
        ...
        train_list_l.append(train_l)
        train_masses.append(dataset_train.masses)
        ...
        if (test_l <= min(np.asarray(train_loss_history)[:,1])) or (test_acc >= max(np.asarray(train_loss_history)[:,4])):
    ...
    print(f'{metric[2] * total_epochs / timer.sum():.1f} examples/sec '
          f'on {str(device)}')
    return train_list_acc,train_list_l,test_list_acc,test_list_l,train_masses
